 ...
 *   const { data: count } = trpc.character.count.useQuery();
 *   // → count = 150
 *
 *   // Server-side
 *   const count = await ctx.character.count();
 *   // → 150
 *
 ...
 *
 * @returns {Promise<number>} Total number of characters (always >= 0)
 */
export const characterCount = async (): Promise<number> => {
  try {
    ...
    logger.error(
      ...
      'Failed to retrieve character count'
    );
